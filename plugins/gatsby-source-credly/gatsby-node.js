...
exports.pluginOptionsSchema = ({ Joi }) => {
  return Joi.object({
    users: Joi.array()
      .required()
      .default([])
      .description('The usernames for Credly accounts to pull data for, e.g. https://www.credly.com/users/{user}/badges.json')
      .external(async opt => {
        if (!opt || opt.length === 0) {
          ...
          for(const user of opt) {
            try {
              ...
              if(response.status !== 200) {
                throw new Error(`Cannot retrieve the badges JSON for user '${user}' (${response.status}). Correct the user and try again!`)
              }
            } catch (err) {
